...
const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addMatcher(adminApi.endpoints.fetchAdmin.matchPending, (state) => {
        state.error = null;
        state.isLoading = true;
      })
      .addMatcher(
    ...
    builder
      .addMatcher(adminApi.endpoints.updateSession.matchPending, (state) => {
        state.error = null;
      })
      .addMatcher(adminApi.endpoints.updateSession.matchFulfilled, () => {})
      .addMatcher(
    ...
    builder
      .addMatcher(adminApi.endpoints.updatePlayer.matchPending, (state) => {
        state.error = null;
      })
      .addMatcher(adminApi.endpoints.updatePlayer.matchFulfilled, () => {})
      .addMatcher(
...
export const { setAdmin, setToken, clearAdmin, clearError, initializeAuth } =
  adminSlice.actions;

// Selectors
export const selectAdmin = (state: { admin: AdminState }) => state.admin.admin;
export const selectIsAuthenticated = (state: { admin: AdminState }) =>
