...
export const createSession = async (
    ...
    try {
        ...
        if (!name || !adminName || !password) {
            return next(new AppError("name, adminName and adminPin are required.", 400));
        }

        const newSession = new Session({ sessionName: name });
        await newSession.save();

        const existingAdmin = await adminModel.findOne({ sessionId: newSession._id });
        if (existingAdmin) {
            return next(new AppError("Admin with this session ID already exists.", 400));
        }
