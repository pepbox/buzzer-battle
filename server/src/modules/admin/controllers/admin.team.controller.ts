    ...
    try {
        ...
        const teamsWithDetails = await Promise.all(
            teams.map(async (team) => {
                const responses = await questionService.fetchResponsesByTeamId(team._id);
                ...
                return {
                    ...
                    status: gameState && gameState.currentAnsweringTeam?.toString() === team._id.toString()
                        ? 'answering'
                        : 'idle',
                };
            })
...
export const updateTeam = async (
    ...
    try {
        ...
        if (teamName === undefined && teamScore === undefined) {
            return next(new AppError("Either teamName or teamScore must be provided.", 400));
        }
        ...
        if (teamScore !== undefined) {
            if (typeof teamScore !== 'number' || teamScore < 0) {
                return next(new AppError("Team score must be a non-negative number.", 400));
            }
